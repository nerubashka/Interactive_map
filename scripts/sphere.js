...
function canvasApp() {
    ...
    function init() {
        ...
        count = wait - 1;
        numToAddEachFrame = 5;
        rgbString = "rgba(255, 255, 255, "; // цвет частиц
        particleAlpha = 1;        
        displayWidth = theCanvas.width = sphere_wrap.offsetWidth;
        displayHeight = theCanvas.height = sphere_wrap.offsetHeight;    
        fLen = 320;
        projCenterX = displayWidth/2; // центр сферы по оси X
        projCenterY = displayHeight; // центр сферы по оси Y
        zMax = fLen-2;
        particleList = {};
        recycleBin = {};
        randAccelX = 0;
        ...
        randAccelZ = 0;
        gravity = -0;
        particleRad = 0.5; // размер частиц
        sphereCenterX = 0;
        sphereCenterY = 0;
        sphereCenterZ = -3 - sphereRad;
        zeroAlphaDepth = -750; 
        turnSpeed = 2*Math.PI/40000; // скорость вращения сферы
        turnAngle = 0; 
    ...
    function onTimer() {    
        ...
        while (p != null) {
            ...
            if (p.dead) {
                ...
                depthAlphaFactor = (depthAlphaFactor<0) ? 1 : 0;
